...
```
```{r}
plot_bar(final_data_con_altitude)
```
...
Con el objetivo entonces de ver qué variables son relevantes, hacemos un mapa de correlación 
```{r}
plot_correlation(na.omit(final_data_con_altitude), maxcat = 5L)
```

### Resultados mapa de calor
Vemos que el mapa de calor ignora las features con más de 5 categorías y en el caso de (certification_address, certification_contact) tienen más de 25. No estoy muy seguro si esto quiere decir que es posible eliminar esas columnas, solo estoy describiendo mi observación.

Lo que sí vi es que hay varias variables con bajísima correlación con total_cup_points como
- Las relacionadas con la altitud, el método de procesamiento, el color verde y las unidades de medida.
